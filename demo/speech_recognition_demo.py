...
def whisper_recognition(
    recognizer: sr.Recognizer, audio: sr.AudioData, model: str, language: str | None = None, prompt: str | None = None
):
    """
    使用 Whisper 识别语音内容
    ...
    """
    try:
        results: str = recognizer.recognize_whisper(
            audio_data=audio,
            model=model,
            language=language,
...
def faster_whisper_recognition(
    audio: sr.AudioData,
    model_size: str,
    ...
    """
    使用 Faster Whisper 识别语音内容

    :param audio: 音频数据
    :param model_size: Faster Whisper 模型
    :param language: 音频内语言, 默认由 Faster Whisper 判断
    :param prompt: 提示词, 默认为空，需要以简体中文展示时使用
    """
    try:
        ...
        segments, _ = model.transcribe(audio_array, language=language, initial_prompt=prompt)
